...
class CustomerPaymentFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(header_frame, text="Receive Customer Payment",
        ...
        ctk.CTkLabel(form_container, text="Customer",
                    font=ctk.CTkFont(size=14, weight="bold"),
                    text_color=XERO_DARK_GRAY).grid(row=1, column=0, padx=(25, 10), pady=8, sticky="w")
        self.customer_combo = ctk.CTkComboBox(form_container, values=[],
                                             font=ctk.CTkFont(size=14),
                                             height=40, corner_radius=8,
                                             command=self.customer_selected)
        self.customer_combo.grid(row=1, column=1, padx=(0, 20), pady=8, sticky="ew")
        ...
        self.payment_method_combo = ctk.CTkComboBox(form_container, 
                                                   values=["Cash", "Bank Transfer", "Cheque", "UPI", "Card"],
                                                   font=ctk.CTkFont(size=14),
                                                   height=40, corner_radius=8)
        self.payment_method_combo.grid(row=2, column=3, padx=(0, 25), pady=(8, 25), sticky="ew")
        ...
        ctk.CTkLabel(title_frame, text="Outstanding Invoices",
                    font=ctk.CTkFont(size=20, weight="bold"),
                    text_color=XERO_NAVY).grid(row=0, column=0, sticky="w")
        ...
        self.tree.heading("invoice_id", text="Invoice #")
        ...
        self.tree.column("invoice_id", width=100, anchor="center")
        ...
        # Payment summary
        summary_frame = ctk.CTkFrame(button_frame, fg_color=XERO_BLUE, corner_radius=8)
    ...
    def save_payment(self):
        ...
        if not all([customer_name, amount_str, payment_date]):
            return messagebox.showerror("Validation Error", "Customer, Payment Amount, and Date are required.")
        ...
        customer = next((c for c in self.customers if c['name'] == customer_name), None)
        if not customer:
            return messagebox.showerror("Error", "Invalid customer selected.")

        invoices_to_pay = db_manager.get_unpaid_sales_invoices(customer['id'])
        if not invoices_to_pay:
            return messagebox.showinfo("No Outstanding Invoices", "This customer has no unpaid invoices.")
        ...
        if payment_id:
            messagebox.showinfo("Payment Recorded", f"Payment successfully recorded with ID: {payment_id}")
